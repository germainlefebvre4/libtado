...
=======
>>>>>>> a8f7e83 (Fixes wrong API call for `activeTimetable`)
  ...
  user_code                = None
    ...
    def set_oauth_token(self, response) -> str:
        ...
        self.refresh_at = datetime.now(
            timezone.utc) + timedelta(seconds=expires_in) - timedelta(seconds=30)
...
>>>>>>> a8f7e83 (Fixes wrong API call for `activeTimetable`)
    ...
    def login_device_flow(self) -> DeviceActivationStatus:
        ...
        expires_in_seconds = response["expires_in"]
        self.device_verification_url_expires_at = datetime.now(
            timezone.utc) + timedelta(seconds=expires_in_seconds)
        # print the expiry time in the user's local timezone.
        device_verification_url_expires_at_local_tz = datetime.now() + \
            timedelta(seconds=expires_in_seconds)

        print(
            "Waiting for you to complete logging in. You have until",
            device_verification_url_expires_at_local_tz.strftime(
                "%Y-%m-%d %H:%M:%S"),
        )
    ...
    def _api_call(self, cmd, data=False, method='GET'):
        ...
        def call_delete(url):
            r = requests.delete(
                url, headers=self.access_headers, timeout=self.timeout)
            r.raise_for_status()
            return r

        def call_put(url, data):
            r = requests.put(url, headers={
                             **self.access_headers, **self.json_content}, data=json.dumps(data), timeout=self.timeout)
            r.raise_for_status()
            return r

        def call_get(url):
            r = requests.get(url, headers=self.access_headers,
    ...
    def get_capabilities(self, zone):
        ...
        """
        data = self._api_call(
            'homes/%i/zones/%i/capabilities' % (self.id, zone))
        return data
    ...
    def set_home_state(self, at_home):
        ...
        self._api_call('homes/%i/presenceLock' %
                       self.id, payload, method='PUT')
    ...
    def get_schedule_timetables(self, zone):
        ...
        data = self._api_call(
            'homes/%i/zones/%i/schedule/timetables' % (self.id, zone))
        return data
    ...
    def get_measuring_device(self, zone):
        ...
        data = self._api_call(
            'homes/%i/zones/%i/measuringDevice' % (self.id, zone))
        return data
    ...
    def get_default_overlay(self, zone):
        ...
        """
        data = self._api_call(
            'homes/%i/zones/%i/defaultOverlay' % (self.id, zone))
        return data
    ...
    def set_zone_name(self, zone, new_name):
        ...
        data = self._api_call('homes/%i/zones/%i/details' %
                              (self.id, zone), payload, method='PUT')
        return data
    ...
    def end_manual_control(self, zone: int):
        ...
        ??? info "Result example"
            ...
            ```
        """
        self._api_call('homes/%i/zones/%i/overlay' %
                       (self.id, zone), method='DELETE')
    ...
    def get_away_configuration(self, zone):
        ...
        data = self._api_call(
            'homes/%i/zones/%i/awayConfiguration' % (self.id, zone))
        return data
    ...
    def set_away_configuration(self, zone, equipment_type, preheating_level, minimumAwayTemperatureCelcius):
        ...
        payload = {'type': equipment_type, 'preheatingLevel': preheating_level,
                   'minimumAwayTemperature': {'celsius': minimumAwayTemperatureCelcius}}

        data = self._api_call('homes/%i/zones/%i/awayConfiguration' %
                              (self.id, zone), data=payload, method='PUT')
        return data
    ...
    def set_open_window_detection(self, zone, enabled, seconds):
        ...
        data = self._api_call('homes/%i/zones/%i/openWindowDetection' %
                              (self.id, zone), data=payload, method='PUT')
        return data
    ...
    def get_report(self, zone, date):
        ...
        """
        data = self._api_call(
            'homes/%i/zones/%i/dayReport?date=%s' % (self.id, zone, date))
        return data
    ...
    def get_air_comfort_geoloc(self, latitude, longitude) -> dict:
        ...
        ??? info "Result example"
            ...
            ```
        """
        data = self._api_acme_call(
            'homes/%i/airComfort?latitude=%f&longitude=%f' % (self.id, latitude, longitude))
        return data
    ...
    def get_running_times(self, from_date):
        ...
        ??? info "Result example"
            ...
            ```
        """
        data = self._api_minder_call(
            'homes/%i/runningTimes?from=%s' % (self.id, from_date))
        return data
    ...
    def get_energy_consumption(self, startDate, endDate, country, ngsw_bypass=True):
        ...
        ??? info "Result example"
            ...
            ```
        """
        data = self._api_energy_insights_call(
            'homes/%i/consumption?startDate=%s&endDate=%s&country=%s&ngsw-bypass=%s' % (self.id, startDate, endDate, country, ngsw_bypass))
        return data
    ...
    def get_energy_savings(self, monthYear, country, ngsw_bypass=True):
        ...
        ??? info "Result example"
            ...
            ```
        """
        data = self._api_energy_bob_call(
            '%i/%s?country=%s&ngsw-bypass=%s' % (self.id, monthYear, country, ngsw_bypass))
        return data
    ...
    def set_cost_simulation(self, country, ngsw_bypass=True, payload=None):
        ...
        data = self._api_energy_insights_call('homes/%i/costSimulator?country=%s&ngsw-bypass=%s' % (
            self.id, country, ngsw_bypass), data=payload, method='POST')
        return data
    ...
    def get_consumption_overview(self, monthYear, country, ngsw_bypass=True):
        ...
        data = self._api_energy_insights_call(
            'homes/%i/consumptionOverview?month=%s&country=%s&ngsw-bypass=%s' % (self.id, monthYear, country, ngsw_bypass))
        return data
    ...
    def get_consumption_details(self, monthYear, ngsw_bypass=True):
        ...
        data = self._api_energy_insights_call(
            'homes/%i/consumptionDetails?month=%s&ngsw-bypass=%s' % (self.id, monthYear, ngsw_bypass))
        return data
    ...
    def get_energy_settings(self, ngsw_bypass=True):
        ...
        data = self._api_energy_insights_call(
            'homes/%i/settings?ngsw-bypass=%s' % (self.id, ngsw_bypass))
        return data
    ...
    def get_energy_insights(self, start_date, end_date, country, ngsw_bypass=True):
        ...
        data = self._api_energy_insights_call(
            'homes/%i/insights?startDate=%s&endDate=%s&country=%s&ngsw-bypass=%s' % (self.id, start_date, end_date, country, ngsw_bypass))
        return data
